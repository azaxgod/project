...
final kguUsersControllerProvider =
    StateNotifierProvider<KguUsersController, AkimatUsersState>((ref) {
  final services = ref.watch(organizationsServicesProvider);
  return KguUsersController(services: services);
});

class KguUsersController extends StateNotifier<AkimatUsersState> implements UsersControllerBase {
  KguUsersController({
  ...
  Future<void> _loadData() async {
    ...
    state = state.copyWith(
      data: await AsyncValue.guard(() async {
        final users = await _services.rolesCollection.getKguUsers();
        return users;
      }),
    );
  }
  ...
  Future<void> createUser({
    ...
    try {
      await _services.rolesCollection.createKguUser(
        phone: phone,
        login: login,
        password: password,
      );
